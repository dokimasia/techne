...
func TestLanguage(t *testing.T) {
	...
	t.Run("Declaration", func(t *testing.T) {
		...
		t.Run("claims the wire form this module owns", func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, string(ruby.Declaration().Language), "ruby",
		...
		t.Run("claims .rb", func(t *testing.T) {
			t.Parallel()
			assert.Contains(t, ruby.Declaration().Extensions, ".rb",
	...
	t.Run("Server", func(t *testing.T) {
		...
		t.Run("can be run as it is declared", func(t *testing.T) {
			...
			assert.NoError(t, ruby.Server().Valid(),
		...
		t.Run("names itself as it names the program to run", func(t *testing.T) {
			...
			assert.Equal(t, ruby.Server().Name, ruby.Server().Command[0],
		...
		t.Run("opens files under the identity the protocol names", func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, ruby.Server().LanguageID, lsp.IdentityRuby,
		...
		t.Run("claims only the roles binding answers", func(t *testing.T) {
			...
			assert.Equal(t, ruby.Server().Reaches(engine.RoleResolve), trust.Resolved,
				"a type checker binds names, which is what resolve asks about")
			assert.Equal(t, ruby.Server().Reaches(engine.RoleOutline), trust.None,
	...
	t.Run("Register", func(t *testing.T) {
		...
		t.Run("puts the language in the registry and its engine in the catalogue", func(t *testing.T) {
			...
			assert.NoError(t, ruby.Register(lang.Workspace{FS: fstest.MapFS{}}, r, c),
				"a composition root registers this module with one call")
			assert.Length(t, r.Languages(), 1, "one call registers one language")
			assert.Length(t, c.For(t.Context(), ruby.Declaration().Language, engine.RoleOutline), 1,
		...
		t.Run("adds the server for a workspace on disk", func(t *testing.T) {
			...
			assert.NoError(t, ruby.Register(held, registry, catalogue),
				"a workspace on disk registers both")

			assert.NotContains(t,
				serving(t, catalogue, ruby.Declaration().Language, engine.RoleOutline),
				ruby.Server().Name,
		...
		t.Run("leaves the server out where a tree is nowhere", func(t *testing.T) {
			...
			registry, catalogue := lang.NewRegistry(), engine.NewCatalog()
			assert.NoError(t, ruby.Register(lang.Workspace{FS: fstest.MapFS{}}, registry, catalogue),
				"a tree that is nowhere still registers a parser")
			assert.NotContains(t,
				serving(t, catalogue, ruby.Declaration().Language, engine.RolePlan),
				ruby.Server().Name,
		...
		t.Run("refuses a second registration of one language", func(t *testing.T) {
			t.Parallel()
			r, c := lang.NewRegistry(), engine.NewCatalog()
			assert.NoError(t, ruby.Register(lang.Workspace{FS: fstest.MapFS{}}, r, c), "the first call registers")
			assert.HasError(t, ruby.Register(lang.Workspace{FS: fstest.MapFS{}}, r, c),
